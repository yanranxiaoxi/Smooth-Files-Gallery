...
function(e) {
	...
	function A(e, t, i) { !! e.offsetParent != !! t && (e.style.display = t ? i ? "" : "block" : "none")
	}
			...
			if (E.clean_localstorage = function() {
				if (!i) {
					...
					e.length && V(e, (function(e) {
						...
						else if (e.startsWith("files:dir:")) if (e.startsWith("files:dir:" + _c.dirs_hash)) {
							if (_c.menu_enabled) {
								var t = e.split(":"),
									i = t[3];
	...
	function() {
		...
		N.plugins = {
			...
			cGF5cGFs: {
				src: [O("aHR0cHM6Ly93d3cucGF5cGFsLmNvbS9zZGsvanM/Y2xpZW50LWlkPQ==") + (location.search.includes("sandbox") ? '' : O("QWQ3TmVxdDQ3eFA5WE1SNk9WaFpoa0tzYm1zZFFOZWw4QTZma1EyQ2JYSFRENmpLMThmUzM2Y3l6ZjBxaUVDVDVmTlgtZkJtOHRJVVNMVHg="))],
				type: "js"
			}
		...
		if (E.breadcrumbs_info = function() {
			e(_c.files_count + " " + (_c.current_dir.images_count === _c.files_count ? G("图像", "图像", _c.files_count) : _c.current_dir.files_count === _c.files_count ? G("文件", "文件", _c.files_count) : !_c.current_dir.files_count && _c.files_count ? G("文件夹", "文件夹", _c.files_count) : G("项目", "项目", _c.files_count)) + (_c.current_dir.dirsize ? "  " + filesize(_c.current_dir.dirsize) : ""))
		}, _c.breadcrumbs) {
	...
	}), "contextmenu"), document.documentElement.style.setProperty("--touch-callout", "none")), function() {
		function e(e, t, i, a) {
			...
			if (_c.context_menu) {
				...
				if (d) {
					...
					if (N.contextmenu.item = l || _c["sidebar" === c ? "dirs" : "files"][v], N.contextmenu.item) {
						o.stopPropagation(), (m || u) && (R.contextmenu.innerHTML = '<h6 class="dropdown-header" title="' + t(N.contextmenu.item.basename) + '">' + E.get_svg_file_icon(N.contextmenu.item) + i(N.contextmenu.item.basename) + "</h6>" + e("缩放", "zoom_in", "popup", "popup" !== c && N.contextmenu.item.is_browser_image && N.contextmenu.item.is_readable) + e("打开", "folder_open", "folder", "sidebar" !== c && "dir" === N.contextmenu.item.filetype) + e("显示信息", "info", "modal", !["modal", "popup"].includes(c)) +
						function(e, t, i, a) {
							if (!e) return "";
							var o = n(e);
							return o && "#" !== o ? g(E.get_svg("open_in_new") + U("在新标签页中打开", i), "a", t, U("open in new tab", a), o) : ""
						}(N.contextmenu.item, "dropdown-item", !0) + e("复制链接", "clipboard", "clipboard", W.url && W.clipboard && N.contextmenu.item.url_path) +
						function(e, t, i, a) {
							return W.download && "dir" !== e.filetype && e.is_readable ? g(E.get_svg("download") + U("下载", i), "a", t, U("download", a), n(e, !0), !0) : ""
		...
		function u(t, i) {
			...
			if (_c.transitions && _c.files_count) {
				...
				for (var n = R.list_ul.children, o = n.length, l = [], p = window.innerHeight, s = 0; s < o; s++) {
					...
					if (!(r.bottom < 0)) if (r.top < p - 10) l.push(c);
		...
		function w() {
			return _c.current_dir.html = M(_c.file_names, (function(e, l) {
				...
				if (_c.load_images && s.is_readable && s.is_browser_image && ("svg" === s.is_browser_image ? s.filesize < _c.load_svg_max_filesize && (f = "svg") : "ico" === s.is_browser_image ? f = "svg" : (s.is_resize_image &&
				function(e) {
				...
				}(s.image) && (x = o), (x || s.filesize <= _c.load_images_max_filesize) && (f = "img", s.image && (u = s.image.width / s.image.height)))), f) var w = '<img class="img img-' + f + ' lazy" data-src="' +
				function() {
		...
		function k(e, t, i) {
			E.topbar_info(E.get_svg("alert_circle_outline") + "<strong>" + G("错误") + "</strong>" + (e ? ": " + e : "."), "error"), i && _c.history && history[i + "State"]({
				path: t
			}, G("错误") + (e ? ": " + e : "."), f(i, t))
		}
		b(R.filter, (function() {
			...
			function p(e) {
				1 == o++ && y(t, i, l ? "JSON 中的文件 " + l : "xmlhttp 中的文件")
			}
		}, E.init_files = function() {
			if (_c.query_path) return _c.query_path_valid ? E.get_files(_c.query_path, "replace") : k("无效的目录路径 <strong>" + _c.query_path + "</strong>", _c.query_path, "replace");
			if (location.search) {
		...
		i.innerHTML = '<button type="button" class="btn-icon btn-topbar">' + E.get_svg("layout_" + N.layout.layout) + '</button><div class="dropdown-menu dropdown-menu-right"><h6 class="dropdown-header">' + G("页面布局") + "</h6>" + M(N.layout.keys, (function(e) {
		...
		})) + '<div class="form-check form-switch cover-toggle">\t  <input class="form-check-input" type="checkbox" id="covertoggle"' + (_c.image_cover ? " checked" : "") + '>\t  <label class="form-check-label" for="covertoggle">' + G("剪裁预览") + "</label>\t</div></div>";
		var a = i.firstChild,
			n = i.lastChild,
	...
	function J() {
		...
		if (!o || o != _c.qrx && O(o) != n) return _c.qrx || !n || n.includes(".") ? !_c.qrx || "string" == typeof _c.qrx && /^[a-f0-9]{32}$/.test(_c.qrx) ? (e = _c.qrx ? "key=" + _c.qrx + "&" : "", t = function(e) {
			...
			params: e + O("YXBwPTEmaG9zdD0=") + encodeURI(n),
			url: O("aHR0cHM6Ly9zbW9vdGhmaWxlc2dhbGxlcnkuc29yYWhhcnUuY29tL2F1dGgv"),
			json_response: !0,
		...
		function l(e, t) {
			var i = Z("license_modal", 0, (e ? '<div class="alert alert-danger alert-dismissible fade show" role="alert"><button class="btn close" data-dismiss="alert">' + E.get_svg("close") + "</button><strong>许可证无效</strong><br>你输入了一个无效的许可证密钥。</div>" : "") + '<div id="pay" class="mb-2"><div id="pay_info" class="mb-3 mt-1"><strong>购买许可证 [$1]</strong> 即可解锁功能并支持我们！\t\t<ul class="ul-reset"><li>' + E.get_svg("check") + "删除此弹出窗口</li><li>" + E.get_svg("check") + "专门的支持服务</li><li>" + E.get_svg("check") + '文件管理器、上传器、代码编辑器、用户、全景查看器等 <strong>[即将到来]</strong></li></ul><small>* 购买后，您将通过电子邮件收到<strong>许可证密钥</strong>。</small><br /><small>* 中国用户请<strong><a href="mailto:purchase@soraharu.com?subject=[购买许可证]%20Smooth%20Files%20Gallery">发送邮件</a></strong>以支付并获取许可证。</small>\t\t</div><div id="pay_pp"></div></div>\t\t<form id="license_form" novalidate>\t\t\t<input id="license_key" class="form-control" type="text" placeholder="许可证密钥" required>\t\t\t<div class="float-right mt-3">\t\t\t\t<button id="continue_free" type="button" class="btn btn-light" data-dismiss="modal">不，谢谢！</button>\t\t\t\t<button id="license_save_button" type="submit" class="btn btn-success text-light" style="display:none;">保存</button>\t\t\t</div>\t\t</form>'),
				n = _id("license_form"),
				o = n.parentNode,
				l = _id("license_key"),
			...
			function m(e) {
				...
				l.classList.toggle("is-valid", o), l.classList.toggle("is-invalid", (t || c) && !o), p.style.display = o ? "" : "none", s.style.display = o ? "none" : "", c && (e.preventDefault(), e.stopPropagation(), o && (S(a, !0), d(1), q({
					...
					complete: function(e, t, a) {
						var o = a && e.success;
						n.innerHTML = '<div class="alert alert-' + (o ? "success" : "danger") + ' mb-0" role="alert">' + E.get_svg(o ? "check" : "alert_circle_outline") + (o ? "许可证已应用，感谢！*.。(๑･∀･๑)*.。" : "无法保存许可证密钥") + "</div>", d(0), o && (_c.qrx = e.md5, setTimeout((function() {
							i.hide()
						}), 2e3))
					}
				})))
			...
			pay_info = _id("pay_info"), key_regex = new RegExp(O("XkYxLVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9JA==")), b(n, m, "input"), b(n, m, "submit"), t && t.Buttons({
				...
				onError: function(e) {
					j("error", e), c.innerHTML = '<div class="alert alert-danger" role="alert">' + E.get_svg("alert_circle_outline") + "付款失败</div>"
				},
				onApprove: function(e, t) {
					return d(1), j("onApprove", e, t), t.order.capture().then((function(e) {
						d(0), c.innerHTML = "<p><strong>" + e.payer.name.given_name + " " + e.payer.name.surname + "</strong><br>" + e.payer.email_address + "</p><p>" + E.get_svg("check") + "感谢您的购买！ 请查看你的邮箱。</p>", j("details", e), S(a, 1)
					}))
				}
			}).render(r)
		}
	...
	_c.check_updates && (S("files:updated") ? (S("files:updated", !0), Z("updated", 0, '<div data-dismiss="modal">' + E.get_svg("check") + "&nbsp; 已成功更新到 <strong>版本 " + _c.version + "</strong>。</div>")) : q({
		...
		complete: function(e, t, i) {
			if (e && t && i && e.hasOwnProperty("success")) {
				var a = e.success;
				if (j(a ? "新版本 " + a + " 可用。" : "已经使用最新版本 " + _c.version), a) {
					if (!W.download && !e.writeable) return void j("下载已禁用，更新不可写");
					_id("change-sort").insertAdjacentHTML("afterend", '<div id="files-notifications" class="dropdown"><button type="button" class="btn-icon btn-topbar">' + E.get_svg("bell") + '</button><div class="dropdown-menu dropdown-menu-right" style="min-width: auto;"><h6 class="dropdown-header">Files ' + a + (e.writeable ? "" : " Update") + '</h6><div style="padding: .5rem 1rem; white-space: nowrap;">' + (W.download ? '<a href="https://cdn.jsdelivr.net/gh/yanranxiaoxi/Smooth-Files-Gallery@' + a + '/index.php" class="btn btn-sm btn-' + (e.writeable ? "secondary" : "primary") + '"' + (e.writeable ? "" : ' style="width:100%"') + " download>" + G("下载") + "</a>" : "") + (e.writeable ? '<button type="button" class="btn btn-primary btn-sm btn-update" style="margin-left: 1px;' + (W.download ? "" : "width:100%;") + '">' + G("更新") + "</button>" : "") + "</div></div></div>");
							...
							confirm("您确定要更新到新版本 " + a + "吗？") && (p(!0, "更新中..."), q({
								...
								complete: function(e, t, i) {
									if (e && t && i && e.hasOwnProperty("success") && e.success) {
										l.innerText = G("已更新！"), S("files:updated", 1);
										try {
											location.reload(!0)
										} catch (e) {
											Z("updated_refresh", 0, E.get_svg("check") + "&nbsp; 成功更新。<strong>请刷新浏览器！</strong>")
										}
									} else l.classList.add("btn-danger"), l.innerText = G("失败"), setTimeout((function() {
			...
			} else j("无法加载外部 JSON check_updates。")
		}
	...
	function() {
		var e = _c.qrx && _c.code_allow_edit,
			t = {};
		R.modal.innerHTML = '<div class="modal-dialog" role="document">\t  <div class="modal-content">\t    <div class="modal-header">\t      <h5 class="modal-title"></h5>\t      <div class="modal-buttons">\t      \t<div class="modal-code-buttons" style="display: none">' + (e ? '<button type="button" class="btn btn-1 is-icon" data-action="save" data-tooltip="' + G("保存") + '">' + E.get_svg("save_edit") + "</button>" : "") + '<button type="button" class="btn btn-1 is-icon" data-action="copy" data-tooltip="' + G("复制代码") + '">' + E.get_svg("clipboard") + '</button><button type="button" class="btn btn-1 is-icon" data-action="fullscreen" data-tooltip="' + G("全屏") + '">' + E.get_svg_multi("expand", "collapse") + '</button></div><button class="btn btn-1 is-icon" data-action="close" data-tooltip="' + G("关闭") + '">' + E.get_svg("close") + '</button>\t      </div>\t    </div>\t    <div class="modal-body"></div>\t  </div>\t</div>';
		...
		E.open_modal = function(o, l) {
			...
			}), "dir" !== o.filetype && o.is_readable) if (o.is_browser_image) {
		...
		var Z = w((function(e) {
			var t = N.modal.code_mirror.getValue(),
				i = t && x(t);
			O(e.target, "btn-1", i ? "btn-success" : "btn-danger", G(i ? "代码已复制" : "复制代码失败"), !0, 2e3)
		}), 2e3);

		function P(e) {
			if (!L.disabled) {
				if (!N.modal.file.is_writeable) return O(L, "btn-1", "btn-danger", G("文件不可写"), !0, 2e3);
				e && L.classList.add("show-tooltip"), A(L, null, null, G("保存中..."), !0), q({
					params: "action=file&file=" + N.modal.file.path + "&write=" + N.modal.code_mirror.getValue(),
					json_response: !0,
					complete: function(e) {
						O(L, "btn-1", e.success ? "btn-success" : "btn-danger", G(e.success ? "已保存" : "保存失败"), !0, 2e3), e.success && S(X(), !0)
					}
				})
			}
		}
	...
	var K = function(e, t) {
			var i, a, n, o, l, p, s, c, r, d, m, u, v, f = this,
				...
				y = function(e) {
					for (var i = 0; i < d.closeElClasses.length; i++) if (t.hasClass(e, "pswp__" + d.closeElClasses[i])) return !0
				},
				H = 0,
				k = function() {
			...
			f.init = function() {
				...
				})), l("parseVerticalMargin", M), function() {
				...
				if (!m) if (i.detail && "mouse" === i.detail.pointerType) {
	...
	function() {
		...
		E.open_popup = function(o, s) {
			if (o && N.list.items.length) {
				...
				if (N.list.matchingItems.forEach((function(e) {
					var t = e._values;
					if (t.is_browser_image && t.is_readable) {
					...
					(x = new PhotoSwipe(R.pswp, K, f, {
						...
						addCaptionHTMLFn: function(e, t) {
							...
							if (y && (y.href = n(i, !0)), L.innerHTML = d(W.is_pointer, "popup-context") + g(i.basename, "div", "popup-basename") + c(i.image, "span", "popup-dimensions", !0) + r(i, "span", "popup-filesize", !0) + g(g(E.get_time(i, M, !1, C), "span", !1, i.DateTimeOriginal ? "原始拍摄时间" : G("修改时间")), "span", "popup-date") + v(i.image, "div", "popup-exif", !0) + m(i.image, "popup"), 1 != getComputedStyle(t).getPropertyValue("opacity")) return !0;
							var a = L.children,
		...
		if (_c.menu_enabled) {
			...
			!(!f || x && !
			function() {
				for (var e = g.length, t = 0; t < e; t++) if (g[t].path.includes("/")) return !1;
				return !0
			}()) ? T(g, "来自本地存储的菜单 [" + (_c.menu_cache_validate ? "浅菜单" : "菜单缓存验证已禁用") + "]") : (R.sidebar_menu.classList.add("sidebar-spinner"), R.sidebar_menu.dataset.title = G("加载中") + " …", q({
				params: !_c.menu_cache_file && "action=dirs" + (_c.cache ? "&menu_cache_hash=" + _c.menu_cache_hash : "") + (f ? "&localstorage=1" : ""),
				url: _c.menu_cache_file,
				json_response: !0,
				complete: function(e, t, i) {
					if (R.sidebar_menu.classList.remove("sidebar-spinner"), delete R.sidebar_menu.dataset.title, !i || !e || e.error || !Object.keys(e).length) return J(), void j("错误或文件夹不存在！");
					e.localstorage ? T(g, "来自本地存储的菜单") : (T(e, "menu from " + (_c.menu_cache_file ? "JSON cache: " + _c.menu_cache_file : "xmlhttp")), W.local_storage && setTimeout((function() {
		...
		function T(t, i) {
			...
			if (J(), j(i, t), _c.dir_paths = [], V(t, (function(e) {
				...
				if (m) {
					...
					V(l, (function(e) {
						if (e.classList.contains("is-open")) if (a) t.push(e);
						else {
			...
			b(a, (function(e) {
				...
				if (e.target !== a) {
					...
					if (!_(e, n)) if (t && n !== r) E.get_files(i.dataset.path, "push"), matchMedia("(min-width: 992px)").matches ? _c.menu_show || h(R.sidebar, "sidebar-clicked", null, 1e3) : C();
		...
		i.innerHTML = '<button type="button" class="btn-icon btn-topbar">' + E.get_svg("sort_" + N.sort.sort) + '</button><div class="dropdown-menu dropdown-menu-right"><h6 class="dropdown-header">' + G("排序方式") + "</h6>" + M(N.sort.keys, (function(e) {
		...
		if (N.topbar = {
			...
			var t = _id("logout");
			t.innerHTML = E.get_svg("logout"), t.dataset.tooltip = G("注销"), t.classList.add("tooltip-left", "tooltip-bottom"), b(t, (function(e) {
				confirm("您确定要注销吗？") || e.preventDefault()
			}))
		}
		var i = _id("topbar-fullscreen");
		screenfull.isEnabled ? (i.innerHTML = E.get_svg_multi("expand", "collapse"), i.dataset.tooltip = G("全屏"), i.classList.add("tooltip-left", "tooltip-bottom"), i.addEventListener("click", (function() {
			screenfull.toggle()
